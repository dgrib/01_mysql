...
-- 4. Практическое задание по теме “CRUD - операции”


/*
1. Заполнить все таблицы БД vk данными (по 10-100 записей в каждой таблице).

-- вставил по 50 записей в каждую таблицу с помощью сайта filldb.info (скрипт приложен в архив ДЗ)
-- проверил скрипт beta в дибивере, работает без ошибок
*/
...
ORDER BY FIRSTNAME ASC; -- можно без ORDER BY, по умолчанию тоже отсортирует список имен в алфавитном порядке

/*
3. Первые пять пользователей пометить как удаленные.
*/

-- я не совсем понял условие, сделал два варианта

-- первые пять пользователей по id
UPDATE users
SET
	is_deleted = 1
LIMIT 5;
-- SELECT firstname, IS_DELETED FROM users; -- проверка

-- первые пять пользователей, отсортированных в алфавитном порядке
UPDATE users
SET
	is_deleted = 1
ORDER BY firstname
LIMIT 5;
-- SELECT firstname, IS_DELETED FROM users ORDER BY FIRSTNAME ASC; -- проверка
...
Тема: Система документооборота на производственном предприятии. (можно расширить до системы управления предприятием, со складами металла и тд)

Проектировщики делают чертежи, участвуют в проектах, к каждому проекту относится несколько чертежей,
чертежи согласовываются (запрос на согласование, согласован, отправлен на доработку) технологами,
технологи выпускают технологии на основании согласованного чертежа
и тд.
*/
